function [Wica_best, Wica_all_initializations, negentropy_all_initializations, negentropy_vs_rotation_all_initializations] = ...
    minimize_via_rotation(Wpca, n_random_initializations, fHandle, fArgs, random_seed, plot_figures)
...
for z = 1:n_random_initializations
    if(verbose)
        dispstat(sprintf('Total Progress = %d/%d',z, n_random_initializations),'keepthis', 'timestamp');
    end
    ...
    while 1
        if(verbose)
            dispstat(sprintf('Subloop [%d]',q),'timestamp');
        end
        ...
        for i = randperm(n_pairs)
            ...
            parfor j = 1:length(th)
                ...
                x = fHandle(Vrot(1,:)',Vrot(2,:)',fArgs{:});
                negentropy_vs_rotation_all_initializations(j,i,z) = x;
            end
            [~,rot(i)] = min(negentropy_vs_rotation_all_initializations(:,i,z));
